            ...
            {!awbShortlist.length ? (
                ...
                <ul>
                    {awbShortlist.map((awb) => (
                        <Box key={awb.id} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
                            <Link to={'/awbShortlist'}>
                                <li>{awb?.fields["HAWB/HBL"]}</li>
                            </Link>
                            <Stack sx={{ marginLeft: '20%' }}>
                                <Button onClick={(e) => handleRemoveAWB(e, awb)} variant="contained">Remove from shortlist</Button>
                            </Stack>
                        </Box>
                    ))}
                </ul>
